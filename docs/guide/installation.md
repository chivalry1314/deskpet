...
每次更新 DeskPet Engine 前，建议先把已有的宠物和配置导出备份：

1. 在宠物管理器中为每个本地宠物点击 **导出**，生成 `.pet` 文件。
2. 或者整体备份本地数据目录（参考 [宠物管理 › 本地数据目录](/guide/manage-pets#本地数据目录)）。

> 更新安装包可能会覆盖或清理旧版数据目录，提前导出 `.pet` 可以避免宠物和配置丢失。更新完成后重新导入即可恢复。
